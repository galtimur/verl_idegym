"""
LangChain ChatModel wrapper for verl's AsyncLLMServerManager.

Bridges verl's token-level LLM serving with LangChain/LangGraph's message-based interface.
Ported from jb-verl/recipe/langgraph_agent/chat_model.py.
...
def convert_to_agent_output(messages: list[BaseMessage], response_length: int) -> AgentLoopOutput:
    ...
    Args:
        messages: List of messages, last message must be assistant
            with response_metadata containing `prompt_ids` and `response_mask`.
        response_length: Max length of response.
